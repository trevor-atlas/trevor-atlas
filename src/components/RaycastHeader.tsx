	...
	React.useEffect(() => {
		...
		function generateParticle(num, amp = 2) {
			...
			for (let i = 1; i < num; i++) {
				...
				// @ts-ignore
				particular.speed =
					particular.position.x + particular.position.z;
		...
		const animate = () => {
			...
			for (let i = 0; i < modularGroup.current.children.length; i++) {
				...
				cube.position.x =
					Math.sin(time * cube.origin.z) * cube.origin.y;

				cube.position.y =
					Math.cos(time * cube.origin.x) * cube.origin.z;

				cube.position.z =
					Math.sin(time * cube.origin.y) * cube.origin.x;
			}
