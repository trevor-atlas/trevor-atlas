...
function Icosahedron({
  ...
  useFrame(({ clock }, delta) => {
    ...
    ref.current.position.x = position.x + Math.sin(step * position.z);
    ref.current.position.y = position.y + Math.cos(step * position.x);
    ref.current.position.z = position.z + Math.sin(step * position.y);
  ...
  return (
    ...
    <group ref={groupRef}>
      {Array(30)
        .fill(0)
        .map((_, i) => {
          const scale = Math.abs(clamp(rand(0.75), 0.1, 0.4));
          const values = {
            position: {
              x: rand(4),
              y: rand(4),
