...
const getContribution = (x: number, y: number, z: number) =>
	0.6 - x * x - y * y - z * z;
...
export class PerlinNoise {
	// This isn't a very good seeding method, but it works ok. It supports 2^16 different seed values
	...
	simplex3(xin: number, yin: number, zin: number) {
		...
		let t0 = getContribution(x0, y0, z0); //0.6 - x0 * x0 - y0 * y0 - z0 * z0;
		if (t0 < 0) {
			n0 = 0;
		} else {
			t0 *= t0;
		...
		let t1 = getContribution(x1, y1, z1); //0.6 - x1 * x1 - y1 * y1 - z1 * z1;
		if (t1 < 0) {
			n1 = 0;
		} else {
			t1 *= t1;
		...
		let t2 = getContribution(x2, y2, z2); //0.6 - x2 * x2 - y2 * y2 - z2 * z2;
		if (t2 < 0) {
			n2 = 0;
		} else {
			t2 *= t2;
		...
		let t3 = getContribution(x3, y3, z3); //0.6 - x3 * x3 - y3 * y3 - z3 * z3;
		if (t3 < 0) {
			n3 = 0;
		} else {
			t3 *= t3;
